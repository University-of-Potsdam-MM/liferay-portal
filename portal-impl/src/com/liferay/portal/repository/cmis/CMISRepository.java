...
public class CMISRepository extends BaseCmisRepository {
	...
	public FileEntry addFileEntry(long folderId, String sourceFileName,
		...
		try {
			...
			org.apache.chemistry.opencmis.client.api.Folder cmisFolder = getCmisFolder(
					session, folderId);
			...
			ContentStream contentStream = new ContentStreamImpl(title,
					BigInteger.valueOf(size), mimeType, is);

			return toFileEntry(cmisFolder.createDocument(properties,
					contentStream, null));
	...
	public Folder addFolder(long parentFolderId, String title,
		...
		try {
			...
			Folder result = toFolder(cmisFolder.createFolder(properties));
			return result;
		} catch (PortalException pe) {
			throw pe;
		} catch (SystemException se) {
	...
	public FileVersion cancelCheckOut(long fileEntryId) throws SystemException {
		...
		try {
			...
			String versionSeriesCheckedOutId = document
					.getVersionSeriesCheckedOutId();

			if (Validator.isNotNull(versionSeriesCheckedOutId)) {
				draftDocument = (Document) session
						.getObject(versionSeriesCheckedOutId);
	...
	public void checkInFileEntry(long fileEntryId, boolean major,
		...
		try {
			...
			_log.error("Unable to check in file entry with {fileEntryId="
					+ fileEntryId + "}", e);
		}
	}
	...
	public FileEntry checkOutFileEntry(long fileEntryId,
		...
		try {
			...
			_log.error("Unable checkout file entry with {fileEntryId="
					+ fileEntryId + "}", e);
		}
	...
	public FileEntry copyFileEntry(long groupId, long fileEntryId,
		...
		try {
			...
			Document newDocument = document.copy(new ObjectIdImpl(
					destFolderObjectId));
	...
	public List<FileEntry> getFileEntries(long folderId, String[] mimeTypes,
		...
		if ((fileEntries == null) || (mimeTypes != null)) {
			...
			List<String> documentIds = getDocumentIds(getSession(), folderId,
					mimeTypes);
	...
	public FileEntry getFileEntry(long folderId, String title)
		...
		try {
			...
			throw new NoSuchFileEntryException(
					"No CMIS file entry with {folderId=" + folderId
							+ ", title=" + title + "}", confe);
		} catch (PortalException pe) {
			throw pe;
		} catch (SystemException se) {
		...
		throw new NoSuchFileEntryException("No CMIS file entry with {folderId="
				+ folderId + ", title=" + title + "}");
	}
	...
	public FileEntry getFileEntryByUuid(String uuid) throws PortalException,
		...
		try {
			...
			throw new NoSuchFileEntryException("No CMIS file entry with {uuid="
					+ uuid + "}", confe);
	...
	public Folder getFolder(long parentFolderId, String title)
		...
		try {
			...
			String objectId = getObjectId(session, parentFolderId, false, title);
		...
		throw new NoSuchFolderException("No CMIS folder with {parentFolderId="
				+ parentFolderId + ", title=" + title + "}");
	}
	...
	public List<Object> getFoldersAndFileEntries(long folderId,
		...
		Map<Long, List<Object>> foldersAndFileEntriesCache = _foldersAndFileEntriesCache
				.get();

		List<Object> foldersAndFileEntries = foldersAndFileEntriesCache
				.get(folderId);

		if ((foldersAndFileEntries == null) || (mimeTypes != null)) {
			foldersAndFileEntries = new ArrayList<Object>(getFolders(folderId));

			List<FileEntry> fileEntries = getFileEntries(folderId, mimeTypes,
	...
	public int getFoldersAndFileEntriesCount(long folderId, String[] mimeTypes)
		...
		if (ArrayUtil.isNotEmpty(mimeTypes)) {
			...
			List<String> documentIds = getDocumentIds(session, folderId,
					mimeTypes);
	...
	public List<String> getObjectPaths(String objectId) throws PortalException,
		...
		throw new RepositoryException("CMIS object is unfileable for id "
				+ objectId);
	}
	...
	public Session getSession() throws PortalException, SystemException {
		...
		SessionImpl sessionImpl = (SessionImpl) _cmisRepositoryHandler
				.getSession();
	...
	public void getSubfolderIds(List<Long> folderIds, long folderId)
		...
		try {
			List<Folder> subfolders = getFolders(folderId, false,
					QueryUtil.ALL_POS, QueryUtil.ALL_POS, null);
	...
	public FileEntry moveFileEntry(long fileEntryId, long newFolderId,
		...
		try {
			...
			document = (Document) document.move(new ObjectIdImpl(
					oldFolderObjectId), new ObjectIdImpl(newFolderObjectId));
	...
	public Folder moveFolder(long folderId, long parentFolderId,
		...
		try {
			...
			org.apache.chemistry.opencmis.client.api.Folder parentCmisFolder = cmisFolder
					.getFolderParent();
			...
			if (!sourceFolderId.equals(targetFolderId)
				...
				cmisFolder = (org.apache.chemistry.opencmis.client.api.Folder) cmisFolder
						.move(new ObjectIdImpl(sourceFolderId),
	...
	public void revertFileEntry(long fileEntryId, String version,
		...
		try {
			...
			updateFileEntry(fileEntryId, contentStream.getFileName(), mimeType,
	...
	public Folder toFolder(String objectId) throws PortalException,
		...
		try {
			...
			throw new NoSuchFolderException("No CMIS folder with {objectId="
					+ objectId + "}", confe);
		} catch (SystemException se) {
			throw se;
		} catch (Exception e) {
	...
	public FileEntry updateFileEntry(long fileEntryId, String sourceFileName,
		...
		try {
			...
			Set<Action> allowableActionsSet = allowableActions
					.getAllowableActions();
			...
			if (allowableActionsSet.contains(Action.CAN_CHECK_OUT)) {
				...
				document = (Document) session
						.getObject(checkOutDocumentObjectId);
			}
			...
			if (is != null) {
				contentStream = new ContentStreamImpl(sourceFileName,
						BigInteger.valueOf(size), mimeType, is);
			}
			...
			if (checkOutDocumentObjectId != null) {
				...
				document.checkIn(majorVersion, properties, contentStream,
						changeLog);
	...
	@Override
	public FileEntry updateFileEntry(String objectId, String mimeType,
			Map<String, Object> properties, InputStream is,
	...
	public Folder updateFolder(long folderId, String title, String description,
		...
		try {
			...
			if (!objectId.equals(newObjectId)) {
				cmisFolder = (org.apache.chemistry.opencmis.client.api.Folder) session
						.getObject(newObjectId);
	...
	protected void cacheFoldersAndFileEntries(long folderId)
		...
		try {
			...
			org.apache.chemistry.opencmis.client.api.Folder cmisParentFolder = getCmisFolder(
					session, folderId);

			Folder parentFolder = toFolder(cmisParentFolder);

			ItemIterable<CmisObject> cmisObjects = cmisParentFolder
					.getChildren();
			...
			Map<Long, List<FileEntry>> fileEntriesCache = _fileEntriesCache
					.get();
	...
	protected void checkUpdatable(Set<Action> allowableActionsSet,
			Map<String, Object> properties, ContentStream contentStream)
	...
	protected void deleteMappedFileEntry(Document document)
		...
		try {
			RepositoryEntryUtil
					.removeByR_M(getRepositoryId(), document.getId());
		} catch (NoSuchRepositoryEntryException nsree) {
		}
	}
	...
	protected void deleteMappedFolder(
		...
		for (CmisObject cmisObject : cmisObjects) {
			if (cmisObject instanceof Document) {
				...
				try {
					RepositoryEntryUtil.removeByR_M(getRepositoryId(),
							cmisObject.getId());

					if (PropsValues.DL_REPOSITORY_CMIS_DELETE_DEPTH == _DELETE_DEEP) {
	...
	protected Hits doSearch(SearchContext searchContext, Query query)
		...
		RepositoryCapabilities repositoryCapabilities = repositoryInfo
				.getCapabilities();

		QueryConfig queryConfig = searchContext.getQueryConfig();

		CapabilityQuery capabilityQuery = repositoryCapabilities
				.getQueryCapability();
		...
		if (productName.contains("Nuxeo") && productVersion.contains("5.4")) {
			queryString += " AND (" + PropertyIds.IS_LATEST_VERSION
					+ " = true)";
		}
		...
		ItemIterable<QueryResult> queryResults = session.query(queryString,
				false);
		...
		for (QueryResult queryResult : queryResults) {
			...
			if (queryConfig.isScoreEnabled()) {
				Object scoreObj = queryResult
						.getPropertyValueByQueryName("HITS");
		...
		float searchTime = (float) (System.currentTimeMillis() - startTime)
				/ Time.SECOND;

		Hits hits = new HitsImpl();

		hits.setDocs(documents
				.toArray(new com.liferay.portal.kernel.search.Document[documents
						.size()]));
	...
	protected Session getCachedSession() {
		...
		TransientValue<Session> transientValue = (TransientValue<Session>) httpSession
				.getAttribute(_sessionKey);
	...
	protected org.apache.chemistry.opencmis.client.api.Folder getCmisFolder(
			Session session, long folderId) throws PortalException,
			SystemException {

		Folder folder = getFolder(session, folderId);

		org.apache.chemistry.opencmis.client.api.Folder cmisFolder = (org.apache.chemistry.opencmis.client.api.Folder) folder
				.getModel();
	...
	protected List<String> getCmisFolderIds(Session session, long folderId)
		...
		ItemIterable<QueryResult> queryResults = session.query(query,
				isAllVersionsSearchableSupported(session));

		List<String> cmsFolderIds = new ArrayList<String>();

		for (QueryResult queryResult : queryResults) {
			PropertyData<String> propertyData = queryResult
	...
	protected List<String> getDocumentIds(Session session, long folderId,
			String[] mimeTypes) throws PortalException, SystemException {
	...
	protected List<Folder> getFolders(long parentFolderId)
		...
		if (folders == null) {
			List<String> folderIds = getCmisFolderIds(getSession(),
					parentFolderId);
	...
	protected String getObjectId(Session session, long folderId,
			boolean fileEntry, String name) throws PortalException,
			SystemException {
	...
	protected void getSubfolderIds(List<Long> subfolderIds,
			List<Folder> subfolders, boolean recurse) throws PortalException,
			SystemException {
		...
		for (Folder subfolder : subfolders) {
			...
			if (recurse) {
				List<Folder> subSubFolders = getFolders(subfolderId, false,
						QueryUtil.ALL_POS, QueryUtil.ALL_POS, null);
	...
	protected void processException(Exception e) throws PortalException {
		if ((e instanceof CmisRuntimeException && e.getMessage().contains(
				"authorized"))
			...
			try {
				message = "Unable to login with user "
	...
	protected <E> List<E> subList(List<E> list, int start, int end,
			OrderByComparator obc) {

		if ((obc != null)
				&& ((obc instanceof RepositoryModelCreateDateComparator)
						|| (obc instanceof RepositoryModelModifiedDateComparator)
						|| (obc instanceof RepositoryModelNameComparator) || (obc instanceof RepositoryModelSizeComparator))) {
	...
	protected FileEntry toFileEntry(Document document, boolean strict)
		...
		FileEntry fileEntry = new CMISFileEntry(this, uuid, fileEntryId,
				document);
	...
	protected String toFolderId(Session session, long folderId)
		...
		repositoryEntry = RepositoryEntryUtil.fetchByR_M(getRepositoryId(),
				rootFolderId);

		if (repositoryEntry == null) {
			repositoryEntry = RepositoryEntryLocalServiceUtil
					.addRepositoryEntry(dlFolder.getUserId(), getGroupId(),
	...
	protected void updateMappedId(long repositoryEntryId, String mappedId)
		...
		RepositoryEntry repositoryEntry = RepositoryEntryUtil
				.findByPrimaryKey(repositoryEntryId);
	...
	private static ThreadLocal<Map<Long, List<FileEntry>>> _fileEntriesCache = new AutoResetThreadLocal<Map<Long, List<FileEntry>>>(
			CMISRepository.class + "._fileEntriesCache",
			new HashMap<Long, List<FileEntry>>());
	private static ThreadLocal<Map<Long, List<Object>>> _foldersAndFileEntriesCache = new AutoResetThreadLocal<Map<Long, List<Object>>>(
			CMISRepository.class + "._foldersAndFileEntriesCache",
			new HashMap<Long, List<Object>>());
	private static ThreadLocal<Map<Long, List<Folder>>> _foldersCache = new AutoResetThreadLocal<Map<Long, List<Folder>>>(
